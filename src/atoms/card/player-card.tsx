...
const PlayerCard = (playerProps : playerProps) => {
    ...
    useEffect(() => {
        if (playerProps.player.id && playerProps.seasons) {
            ...
            GetPlayerGameStats(playerProps.player.id).then(stats => {
                const filteredStats = stats?.filter((stat: GameStat) =>
    ...
    const getSeasonStats = () => {
        ...
        return Array.from(seasonStatsMap.values()).sort((a, b) => {
            const aDate = new Date(a.season.startDate || `${a.season.monthStart} 1, 2020`);
            const bDate = new Date(b.season.startDate || `${b.season.monthStart} 1, 2020`);
    ...
    const getPositionColor = () => {
        switch (playerProps.player.position) {
            case 1: return 'warning'; // GK
            case 2: case 3: case 4: case 5: return 'primary'; // Defenders
            case 6: case 7: case 8: return 'info'; // Midfielders
            case 9: case 10: case 11: return 'danger'; // Forwards
                ...
                <div className="card-body p-3">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        <div className="flex-grow-1">
                            <h6 className="mb-0 text-success fw-bold" style={{ fontSize: '0.95rem' }}>
                                {playerProps.player.firstname} {playerProps.player.surname}
                            </h6>
                        </div>
                        <div className="d-flex gap-1 align-items-center">
                            <span className={`badge bg-${getPositionColor()} text-white`} style={{ fontSize: '0.7rem' }}>
                                {position}
                            </span>
                            <span className="badge bg-dark text-white" style={{ fontSize: '0.7rem' }}>
                    ...
                    <div>
                        ...
                        <div className="row g-1 mb-1 pb-1 border-bottom border-light">
                            <div className="col-4">
                                <small className="text-muted fw-bold" style={{ fontSize: '0.7rem' }}>Season</small>
                            </div>
                            <div className="col-2 text-center">
                                <small className="text-muted fw-bold" style={{ fontSize: '0.7rem' }}>Apps</small>
                            </div>
                            <div className="col-2 text-center">
                                <small className="text-muted fw-bold" style={{ fontSize: '0.7rem' }}>Gls</small>
                            </div>
                            <div className="col-2 text-center">
                                <small className="text-muted fw-bold" style={{ fontSize: '0.7rem' }}>Ast</small>
                            </div>
                            <div className="col-2 text-center">
                                <small className="text-muted fw-bold" style={{ fontSize: '0.7rem' }}>SOT</small>
                            </div>
                        </div>

                        {/* Stats Rows */}
                        {seasonStats.map((stats, index) => (
                            <div key={stats.season.id} className="row g-1 mb-1 align-items-center">
                                <div className="col-4">

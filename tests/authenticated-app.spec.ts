...
test.describe('NPTFC Authenticated App Verification', () => {
  ...
  test('should be authenticated and show app content', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/authenticated-home.png', fullPage: true });
  ...
  test('should access admin page and see what content is available', async ({ page }) => {
    // Navigate to admin
    ...
    await page.screenshot({ path: 'test-results/admin-page-layout.png', fullPage: true });
    ...
    console.log('Admin page text content (first 500 chars):', pageText?.substring(0, 500));
  ...
  test('should try accessing different admin routes directly', async ({ page }) => {
    const adminRoutes = [
      '/admin',
      '/AdminSeason',
      '/AdminPlayer',

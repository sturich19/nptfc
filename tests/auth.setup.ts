...
setup("authenticate", async ({ page }) => {
  ...
  if (!email || !password) {
    throw new Error(
      "AUTH0_TEST_EMAIL and AUTH0_TEST_PASSWORD must be set in .env.test",
    );
  }
  ...
  if (!currentUrl.includes("auth0.com")) {
    ...
    try {
      ...
      console.log(
        "This might mean user is already authenticated or Auth0 config issue",
      );
  ...
  // Wait for app to be ready (don't wait for networkidle as backend API calls may keep failing)
  try {
    await page.waitForLoadState("load", { timeout: 10000 });
  } catch {
    console.log('⚠️ Page did not reach "load" state, but continuing...');
  }

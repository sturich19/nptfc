...
test.describe('NPTFC App Loading', () => {
  test('should load the application and handle authentication', async ({ page }) => {
    ...
    if (currentUrl.includes('auth0.com')) {
      ...
      if (!loginFormFound) {
        console.log('Login form structure may have changed, but Auth0 redirect is working');
        expect(currentUrl).toContain('auth0.com');
      }
  ...
  test('should have correct page title', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/NPTFC|Newport Pagnell Tigers|React App/);
  });
  ...
  test('should not have critical JavaScript errors on load', async ({ page }) => {
    ...
    // Filter out expected Auth0 and known issues

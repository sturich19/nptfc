...
|------|----------|--------------|---------|
| **Sprint Planning** | `polaris-specs/{{feature_slug}}/research/` | Main repo | Evidence/sources for planning THIS sprint |
...
python -c "import json; print(json.load(open('polaris-specs/{{feature_slug}}/meta.json')).get('deliverables_path','(not set)'))"
```
...
- `polaris-specs/` (reserved for sprint planning)
- `research/` at project root (ambiguous, conflicts with polaris-specs/###/research/)
...
python -c "from pathlib import Path; Path('{{deliverables_path}}').mkdir(parents=True, exist_ok=True)"
...
Planning artifacts in `polaris-specs/{{feature_slug}}/research/` are:
...
echo '2025-01-25T14:00:00,journal,"Smith, J. (2024). AI Tools. Nature, 10(2), 123.",AI improves productivity 30%,high,Meta-analysis' \
  >> polaris-specs/{{feature_slug}}/research/evidence-log.csv
```
...
echo 'S001,"Smith (2024). AI Tools.",https://example.com,2025-01-25,high,reviewed' \
  >> polaris-specs/{{feature_slug}}/research/source-register.csv
```

**Why this matters:**
- Schema validation runs during `/polaris.review`
...
| **Merges to main** | Yes, via Polaris merge | Yes, via Polaris merge |
| **Planning artifacts** | N/A | `polaris-specs/.../research/` (in main) |

### Why This Changed

Previously, research artifacts went in `polaris-specs/` which is sparse-excluded from worktrees. This meant:
...
### Mistake 1: Putting Deliverables in polaris-specs/

**Wrong**:
```bash
# Creating files in planning artifacts location
echo "# Findings" > polaris-specs/{{feature_slug}}/findings.md  # BAD!
```
...
# Edit deliverables
# Immediately run:
polaris agent tasks move-task {{wp_id}} --to for_review  # BAD! Nothing committed!
```
...
Co-Authored-By: Aptean Polaris <polaris@aptean.com>"
polaris agent tasks move-task {{wp_id}} --to for_review  # GOOD!
```
...
Since deliverables go in `{{deliverables_path}}`, not `polaris-specs/`:
...
- [ ] All subtasks marked as done: `polaris agent tasks mark-status T### --status done`

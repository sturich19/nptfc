...
# Command Template: /polaris.tasks (Research Mission)
...
**Note**: Task generation in the target branch is standard for all Polaris missions. Implementation happens in per-WP worktrees.

---

## Outline

1. **Setup**: Run `polaris agent feature check-prerequisites --json --paths-only --include-tasks`

   **CRITICAL**: The command returns JSON with `FEATURE_DIR` as an ABSOLUTE path (e.g., `/Users/robert/Code/project/polaris-specs/015-research-topic`).
...
6. **Generate prompt files**:
   ...
   **IMPORTANT**: All WP files live in flat `tasks/` directory. Lane status is tracked ONLY in the `lane:` frontmatter field, NOT by directory location. Agents can change lanes by editing the `lane:` field directly or using `polaris agent tasks move-task`.
...
8. **Report**:
   ...
   - Next command: `/polaris.implement WP01`
...
lane: "planned"  # DO NOT EDIT - use: polaris agent tasks move-task <WPID> --to <lane>
assignee: ""
agent: ""
shell_pid: ""
review_status: ""
...
**DO NOT use markdown status fields like `**Status**: planned`**. The `lane:` field in YAML frontmatter is the ONLY status tracking mechanism. The review command (`/polaris.review`) searches for `lane: "for_review"` in frontmatter to find WPs ready for review.
...
- Use `polaris agent tasks move-task` to change lanes

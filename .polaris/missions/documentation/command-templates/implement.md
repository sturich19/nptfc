...
**Steps**:
1. Create docs/ directory structure (tutorials, how-to, reference/api, explanation):
   ```
   python -c "from pathlib import Path; [Path(d).mkdir(parents=True,exist_ok=True) for d in ['docs/tutorials','docs/how-to','docs/reference/api','docs/explanation']]"
   ```
...
4. Create build script (choose based on platform):

   **Python (cross-platform) - build_docs.py:**
   ```python
   """Build documentation using configured generators."""
   import subprocess, sys
   generators = [
       (["sphinx-build", "-b", "html", "docs/", "docs/_build/html/"], "Sphinx"),
       (["npx", "jsdoc", "-c", "jsdoc.json"], "JSDoc"),
       (["cargo", "doc", "--no-deps"], "rustdoc"),
   ]
   for cmd, name in generators:
       try:
   ...
   ```
   Run only the generators relevant to the project.
...
- build_docs.py script (cross-platform)
...
**Steps**:
1. **Automated checks** (use your agent's search tools or these cross-platform commands):

   - **Heading hierarchy**: Search all `docs/**/*.md` files for lines starting with `#`. Verify headings follow proper hierarchy (no skipping levels).
   - **Broken links**: `npx markdown-link-check docs/**/*.md` (npm, cross-platform)
   - **Missing alt text**: Search `docs/` for image references `![...](...` where the alt text between `[]` is empty.
   - **Spell check**: Use your editor's built-in spell checker, or `npx cspell docs/**/*.md` (cross-platform).
   - **Build check**: `python build_docs.py` - verify no errors in output.
...
6. **Final build and deploy** (if applicable):
   ...
   ```
   Deploy to hosting platform as needed (GitHub Pages, ReadTheDocs, etc.).
...
If `/polaris.status` shows your WP in "doing" after you moved it to "for_review", don't panic - a reviewer may have moved it back (changes requested), or there's a sync delay. Focus on your WP.

...
**Note**: Planning in main is standard for all Polaris missions. Implementation happens in per-WP worktrees.
...
1. **Setup**: Run `polaris agent feature setup-plan --json` to initialize plan.md
...
4. **Phase 1: Design**
   ...
   **Output**: Generator config snippets in plan.md, templates ready for implementation
   ...
   Outline high-level work packages (detailed in `/polaris.tasks`):
...
- Work breakdown will be detailed in `/polaris.tasks`

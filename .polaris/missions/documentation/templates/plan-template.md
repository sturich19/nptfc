...
**Note**: This template is filled in by the `/polaris.plan` command. See mission command templates for execution workflow.
...
└── tasks.md             # Phase 2 output (/polaris.tasks command)
```
...
**Note**: Phase 2 (work package generation) is handled by the `/polaris.tasks` command.

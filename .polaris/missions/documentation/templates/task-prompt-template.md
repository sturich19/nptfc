...
> **Populated by `/polaris.review`** – Reviewers add detailed feedback here when work needs changes. Implementation must address every item listed below before returning for re-review.
...
- Link to supporting specs: `.polaris/memory/constitution.md`, `polaris-specs/.../plan.md`, `polaris-specs/.../tasks.md`, data model, contracts, research, quickstart.
...
- Key acceptance checkpoints for `/polaris.review`.
...
2. **Use CLI**: `polaris agent tasks move-task <WPID> --to <lane> --note "message"` (recommended)

The CLI command updates both frontmatter and activity log automatically.

**Valid lanes**: `planned`, `doing`, `testing`, `for_review`, `done`

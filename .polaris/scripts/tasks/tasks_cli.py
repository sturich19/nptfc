#!/usr/bin/env python3
"""CLI utilities for managing Polaris work-package prompts and acceptance."""
...
def _collect_summary_with_encoding(
    ...
    *,
    ...
    try:
        ...
        if cleaned:
            print("[polaris] Normalized artifact encoding for:", file=sys.stderr)
            for path in cleaned:
                try:
            ...
            print(
                "[polaris] normalize-encoding enabled but no files required updates.",
                file=sys.stderr,
            )
        return collect_feature_summary(
            repo_root,
...
def _check_legacy_format(feature: str, repo_root: Path) -> bool:
    ...
    if is_legacy_format(feature_path):
        if not _legacy_warning_shown:
            ...
            print("Run `polaris upgrade` to migrate to frontmatter-only lanes.", file=sys.stderr)
            print("", file=sys.stderr)
...
def update_command(args: argparse.Namespace) -> None:
    ...
    if _check_legacy_format(feature, repo_root):
        ...
        print("Run 'polaris upgrade' first, then retry.", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"[OK] Updated {wp.work_package_id or wp.path.name} -> {validated_lane}")
...
def list_command(args: argparse.Namespace) -> None:
    repo_root = find_repo_root()
    feature_path = repo_root / "polaris-specs" / args.feature
...
def accept_command(args: argparse.Namespace) -> None:
    ...
    print(f"[OK] Feature '{feature}' accepted at {result.accepted_at} by {result.accepted_by}")
...
def _prepare_merge_metadata(
    ...
    feature_dir = repo_root / "polaris-specs" / feature
...
def merge_command(args: argparse.Namespace) -> None:
    ...
    if args.push and has_remote:
        ...
        print("[polaris] Skipping push: no remote configured.", file=sys.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Polaris task utilities")

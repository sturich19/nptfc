...
def main():
    ...
    if not feature_paths:
        ...
        print(f"  Main specs: {project_dir / 'polaris-specs'} exists: {(project_dir / 'polaris-specs').exists()}")
        ...
        if (project_dir / '.worktrees').exists():
            for wt_dir in (project_dir / '.worktrees').iterdir():
                if wt_dir.is_dir():
                    wt_specs = wt_dir / 'polaris-specs'
                    print(f"    {wt_dir.name}/polaris-specs exists: {wt_specs.exists()}")
                    if wt_specs.exists():

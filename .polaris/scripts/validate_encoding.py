#!/usr/bin/env python3
"""
Validate and fix UTF-8 encoding in Polaris markdown files.
...
Usage:
    python -m specify_cli.scripts.validate_encoding --check polaris-specs/
    python -m specify_cli.scripts.validate_encoding --fix polaris-specs/001-feature/
    python -m specify_cli.scripts.validate_encoding --scan-all
"""
...
def convert_to_utf8(file_path: Path, source_encoding: str = 'windows-1252', dry_run: bool = False) -> bool:
    ...
    try:
        ...
        # Decode with source encoding using strict mode to detect issues
        try:
            text = data.decode(source_encoding, errors='strict')
        except UnicodeDecodeError as decode_err:
            print(f"  Warning: Could not decode with {source_encoding} at byte {decode_err.start}")
            text = data.decode(source_encoding, errors='backslashreplace')
...
def main():
    ...
    parser.add_argument('path', nargs='?', default='polaris-specs', help='Path to scan (default: polaris-specs)')
